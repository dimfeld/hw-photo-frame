...
An empty library returns `204`, with `X-Display-Seconds` and no body. The frame keeps its image and uses this time before its next request. A failed request also leaves the image intact. The frame advances its local cursor only after it receives a complete valid image.
...
| PUT /api/settings | JSON: seconds, fit (`contain` or `cover`), ordering (`sequential` or `random`) |
...
The photo time must be a positive whole number of seconds. The upper bound is derived from JavaScript's safe integer range after conversion to microseconds. The firmware splits long waits at the FreeRTOS tick range.
